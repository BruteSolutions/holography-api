 ...
 */
ColorTranslator::ColorTranslator() throws ( std::string ) {
 ...
 */
ColorTranslator::ColorTranslator( Vec3 factor ) throws ( std::string ) {
    setConversionFactor( factor );
    shader = new Shader( "main.vert","main.frag" );
}
 ...
 * that will be converted to red.
 */
void ColorTranslator::setConversionFactor( Vec3 factor ) throws ( std::string ) {
    if( fac.x < 0 || fac.y < 0 || fac.z < 0 )
        throw std::string("IllegalArgumentException(A conversion-factor is less than 0.)");
    float factorSum = fac.x + fac.y + fac.z;
    if( factorSum > 1 ){ //normalize components so that the sum will be one
    	fac.x = fac.x / factorSum;
        fac.y = fac.y / factorSum;
        fac.z = fac.z / factorSum;
    }
    this->factor = factor;
}
