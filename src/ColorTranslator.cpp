...
void ColorTranslator::setConversionFactor(Vec3 factor) throw (std::string) {
	if (factor.x < 0)
		throw std::string(
				"IllegalArgumentException(The first conversion-factor is less than 0.)");
	if (factor.y < 0)
		throw std::string(
				"IllegalArgumentException(The second conversion-factor is less than 0.)");
	if (factor.z < 0)
		throw std::string(
				"IllegalArgumentException(The third conversion-factor is less than 0.)");
...
void ColorTranslator::setFactorUniform() {
	GLuint factorLocation = glGetUniformLocation(shader->getShaderProgram(),
			"factor");
