...
void main()
{
	...
	vec4 rotated = objectRotY*objectRotZ*objectRotX*setOrigin;
		//Restore position
	vec4 position2 = rotated - vec4(objOrigin.x,objOrigin.y,objOrigin.z, 0);
		//Scale the object
	...
	position2 -= vec4(worldOrigin.x, worldOrigin.y, worldOrigin.z, 0);
		//Add translations
	position2 += vec4(objectPos.x, objectPos.y, objectPos.z, 0) + vec4(camPos.x, camPos.y,camPos.z,0);
		//Perspective matrix
